$(document).ready(function () {
	...
	function two_players () {
		...
		$buttons_row_1_child
			.after(
				"<button id = 'submit' style = 'width: 290px; margin: 10px 0 30px 0;'>Here're Our Names</button>"
			)
			.after(
				"<input id = 'player-1' style = 'display: block;' placeholder='Player 1 First Name'</input>" +
					"<input id = 'player-2' style = 'display: block;' placeholder='Player 2 First Name'</input>"
			);
		...
		$('#submit').click(function () {
			...
			$pick_color.html(
				'Hello, ' +
					...
					'</br></br>' +
					$('#player-1').val() +
					" is <span style='color: #4189C7;'>Blue</span>, and " +
					$('#player-2').val() +
					" is <span style='color: #C73D47;'>Red</span>.</br>" +
					$('#player-1').val() +
					', go first!'
			);
	...
	function one_player () {
		...
		$buttons_row_1_child
			.after("<button id = 'submit'>Here's My Name</button>")
			.after("<input placeholder='First Name'</input>");
		...
		$('#submit').click(function () {
			...
			// populate the pick-color h2 with the name of the user and with the question of what color they want to be.
			$pick_color.html(
				'Hello, ' + $('input').val() + '! ' + 'Are you feeling confident?'
			);
