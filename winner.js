function check_for_win (column_name, winning_mark) {
	...
	if (moves === 42) {
		...
		setTimeout(function () {
			$buttons_row.append(
				"<h1 style='font-size: 6em; margin: 13vh 0 25px 0'>It's a Draw!</h1>"
			);
	...
	for (var i = 0; i < 7; i++) {
		if (column_name === 'col-' + i) {
			...
			for (var j = column_arr.length - 1; j > 1; j--) {
				...
				if (
					column_arr[j].getAttribute('data-name') === winning_mark &&
...
function announce_winner (winning_mark) {
	if (winning_mark === player_1.data_name) {
		...
		setTimeout(function () {
			$buttons_row
				.append(
					"<h1 style='font-size: 6em; margin: 13vh 0 25px 0'>" +
						winning_mark +
						' wins!</h1>'
				)
				.fadeIn();
		...
		setTimeout(function () {
			$buttons_row
				.append(
					"<h1 style='font-size: 6em; margin: 13vh 0 25px 0'>Computer wins!</h1>"
				)
				.fadeIn();
			play_again();
		}, 500);
