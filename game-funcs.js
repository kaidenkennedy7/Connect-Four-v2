			...
			if ($(this).attr('data-name') === 'nothing') {
				...
				if (this_player_clicked === 1) {
					check_spot(
						...
						'circle-background-color-' + player_1.mark_color,
						player_1.data_name
					);
					...
					check_spot(
						$(this).attr('class').split(' ')[1],
						'circle-background-color-' + player_2.mark_color,
						player_2.data_name
					);
	...
	$.each($all_circles, function (index, value) {
		$all_circles.eq(index).click(function () {
			if ($(this).attr('data-name') === 'nothing') {
				...
				check_spot(
					$(this).attr('class').split(' ')[1],
					'circle-background-color-' + player.mark_color,
					player.data_name
				);
...
function computer_move () {
	...
	if ($all_circles.eq(computer_choice).attr('data-name') === 'nothing') {
		...
		check_spot(
			$all_circles.eq(computer_choice).attr('class').split(' ')[1],
			'circle-background-color-' + computer.mark_color,
			computer.data_name
		);
